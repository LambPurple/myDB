...
TEST_F(EnvPosixTest, TestOpenOnRead) {
  ...
  // open-on-read behavior of POSIX Env mydb::RandomAccessFile.
  const int kNumFiles = kReadOnlyFileLimit + kMMapLimit + 5;
  mydb::RandomAccessFile* files[kNumFiles] = {0};
  for (int i = 0; i < kNumFiles; i++) {
    ASSERT_MYDB_OK(env_->NewRandomAccessFile(test_file, &files[i]));
  }
  char scratch;
  Slice read_result;
  for (int i = 0; i < kNumFiles; i++) {
    ASSERT_MYDB_OK(files[i]->Read(i, 1, &read_result, &scratch));
    ASSERT_EQ(kFileData[i], read_result[0]);
  }
  for (int i = 0; i < kNumFiles; i++) {
    delete files[i];
  }
  ASSERT_MYDB_OK(env_->RemoveFile(test_file));
}
...
TEST_F(EnvPosixTest, TestCloseOnExecSequentialFile) {
  ...
  ASSERT_MYDB_OK(WriteStringToFile(env_, "0123456789", file_path));

  mydb::SequentialFile* file = nullptr;
  ASSERT_MYDB_OK(env_->NewSequentialFile(file_path, &file));
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;

  ASSERT_MYDB_OK(env_->RemoveFile(file_path));
}
...
TEST_F(EnvPosixTest, TestCloseOnExecRandomAccessFile) {
  ...
  // mmap region.
  mydb::RandomAccessFile* mmapped_files[kMMapLimit];
  for (int i = 0; i < kMMapLimit; i++) {
    ASSERT_MYDB_OK(env_->NewRandomAccessFile(file_path, &mmapped_files[i]));
  }

  mydb::RandomAccessFile* file = nullptr;
  ASSERT_MYDB_OK(env_->NewRandomAccessFile(file_path, &file));
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;
...
TEST_F(EnvPosixTest, TestCloseOnExecWritableFile) {
  ...
  mydb::WritableFile* file = nullptr;
  ASSERT_MYDB_OK(env_->NewWritableFile(file_path, &file));
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;
...
TEST_F(EnvPosixTest, TestCloseOnExecAppendableFile) {
  ...
  mydb::WritableFile* file = nullptr;
  ASSERT_MYDB_OK(env_->NewAppendableFile(file_path, &file));
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;
...
TEST_F(EnvPosixTest, TestCloseOnExecLockFile) {
  ...
  mydb::FileLock* lock = nullptr;
  ASSERT_MYDB_OK(env_->LockFile(file_path, &lock));
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
...
TEST_F(EnvPosixTest, TestCloseOnExecLogger) {
  ...
  mydb::Logger* file = nullptr;
  ASSERT_MYDB_OK(env_->NewLogger(file_path, &file));
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;
...
int main(int argc, char** argv) {
  ...
  mydb::EnvPosixTest::SetFileLimits(mydb::kReadOnlyFileLimit,
                                       mydb::kMMapLimit);
