...
class ReverseKeyComparator : public Comparator {
 public:
  const char* Name() const override {
    return "mydb.ReverseBytewiseComparator";
  }
...
// Special test for a block with no restart entries.  The C++ mydb
// code never generates such blocks, but the Java version of mydb
// seems to.
...
TEST_F(Harness, RandomizedLongDB) {
  ...
  for (int level = 0; level < config::kNumLevels; level++) {
    std::string value;
    char name[100];
    std::snprintf(name, sizeof(name), "mydb.num-files-at-level%d", level);
