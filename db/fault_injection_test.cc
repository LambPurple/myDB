// Copyright 2014 The MyDB Authors. All rights reserved.
 ...
 public:
  ...
  void DeleteAllData() {
    ...
    for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
      ASSERT_MYDB_OK(db_->Delete(WriteOptions(), iter->key()));
    }
  ...
  void ResetDBState(ResetMethod reset_method) {
    switch (reset_method) {
      case RESET_DROP_UNSYNCED_DATA:
        ASSERT_MYDB_OK(env_->DropUnsyncedFileData());
        break;
      case RESET_DELETE_UNSYNCED_FILES:
        ASSERT_MYDB_OK(env_->RemoveFilesCreatedAfterLastDirSync());
        break;
      default:
        assert(false);
    }
  ...
  void PartialCompactTestReopenWithFault(ResetMethod reset_method,
    ...
    ASSERT_MYDB_OK(OpenDB());
    ASSERT_MYDB_OK(
        Verify(0, num_pre_sync, FaultInjectionTest::VAL_EXPECT_NO_ERROR));
    ASSERT_MYDB_OK(Verify(num_pre_sync, num_post_sync,
