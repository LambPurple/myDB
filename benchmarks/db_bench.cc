...
void Uncompress(
    ...
    while (ok && bytes < 1024 * 1048576) { // Compress 1G
        ok =
            uncompress_func(compressed.data(), compressed.size(), uncompressed);
        bytes += input.size();
        thread->stats.FinishedSingleOp();
    }
    delete[] uncompressed;
...
class Benchmark {
  private:
    ...
    void PrintWarnings() {
        ...
        std::fprintf(stdout, "WARNING: Optimization is disabled: benchmarks "
                             "unnecessarily slow\n");
#endif
#ifndef NDEBUG
        std::fprintf(
    ...
    void Run() {
        ...
        while (benchmarks != nullptr) {
            ...
            if (name == Slice("open")) {
                ...
                PrintStats("mydb.stats");
            } else if (name == Slice("sstables")) {
                PrintStats("mydb.sstables");
            } else {
...
int main(int argc, char** argv) {
    FLAGS_write_buffer_size = mydb::Options().write_buffer_size;
    FLAGS_max_file_size = mydb::Options().max_file_size;
    FLAGS_block_size = mydb::Options().block_size;
    FLAGS_open_files = mydb::Options().max_open_files;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        int n;
        char junk;
        if (mydb::Slice(argv[i]).starts_with("--benchmarks=")) {
    ...
    mydb::g_env = mydb::Env::Default();

    // Choose a location for the test database if none given with --db=<path>
    if (FLAGS_db == nullptr) {
        mydb::g_env->GetTestDirectory(&default_db_path);
        default_db_path += "/dbbench";
        FLAGS_db = default_db_path.c_str();
    }

    mydb::Benchmark benchmark;
    benchmark.Run();
    return 0;
}
