...
template <size_t SIZE> struct HashT {
    ...
    std::string to_string(size_t num_bytes = SIZE,
  ...
  public:
        ...
        for (const Element& e : elements) {
            if (e.direction == PATH_LEFT) {
                MERKLECPP_TRACE(MERKLECPP_TOUT
                                << " - " << e.hash.to_string(TRACE_HASH_SIZE)
    ...
    size_t serialised_size() const {
        return sizeof(_leaf) + sizeof(uint64_t) +         // leaf index
               ...
               elements.size() * (sizeof(Element::hash) + // hash
    ...
    bool operator==(const PathT<HASH_SIZE, HASH_FUNCTION>& other) const {
        ...
        while (it != elements.end() && other_it != other.elements.end()) {
            if (it->hash != other_it->hash ||
                it->direction != other_it->direction)
                return false;
            it++;
            other_it++;
        }
...
class TreeT {
    ...
    struct Node {
        ...
        static Node* copy_node(const Node* from,
            ...
            r->left = copy_node(from->left, leaf_nodes, num_flushed, min_index,
                                max_index, indent + 1);
            r->right = copy_node(from->right, leaf_nodes, num_flushed,
                                 min_index, max_index, indent + 1);
  ...
  public:
    ...
    void retract_to(size_t index) {
        MERKLECPP_TRACE(MERKLECPP_TOUT << "> retract_to " << index
                                       << std::endl;);
        ...
        Node* new_leaf_node =
            walk_to(index, true, [this](Node*& n, bool go_right) {
                ...
                n->dirty = true;
                if (go_left && n->right) {
                    MERKLECPP_TRACE(MERKLECPP_TOUT << " - eliminate "
                                                   << n->right->hash.to_string(
                                                          TRACE_HASH_SIZE)
                                                   << std::endl;);
    ...
    const Hash& root() {
        ...
        MERKLECPP_TRACE(MERKLECPP_TOUT << " - root: "
                                       << _root->hash.to_string(TRACE_HASH_SIZE)
                                       << std::endl;);
        return _root->hash;
    }
    ...
    std::shared_ptr<Hash> past_root(size_t index) {
        MERKLECPP_TRACE(MERKLECPP_TOUT << "> past_root " << index
                                       << std::endl;);
        ...
        MERKLECPP_TRACE(MERKLECPP_TOUT << " - " << p->to_string(TRACE_HASH_SIZE)
                                       << std::endl;
                        MERKLECPP_TOUT << " - "
                                       << result->to_string(TRACE_HASH_SIZE)
                                       << std::endl;);
    ...
    std::shared_ptr<Path> path(size_t index) {
        MERKLECPP_TRACE(MERKLECPP_TOUT << "> path from " << index
                                       << std::endl;);
    ...
    std::shared_ptr<Path> past_path(size_t index, size_t as_of) {
        ...
        for (uint8_t height = _root->height; height > 1;) {
            ...
            MERKLECPP_TRACE(
                MERKLECPP_TOUT
                    ...
                    << cur_a->size << "/" << (unsigned)cur_a->height << "/"
            ...
            if (!fork_node) {
                ...
                if (cur_i->height == height) {
                    if (go_right_i) {
                        typename Path::Element e;
                        e.hash =
                            go_right_i ? cur_i->left->hash : cur_i->right->hash;
                        e.direction =
                            go_right_i ? Path::PATH_LEFT : Path::PATH_RIGHT;
                ...
                if (cur_a->height == height) {
                    // The right path does not take into account anything to the
                    // right of `as_of`, as those nodes were inserted into the
                    // tree after `as_of`.
                    if (go_right_a) {
                        typename Path::Element e;
                        e.hash =
                            go_right_a ? cur_a->left->hash : cur_a->right->hash;
                        e.direction =
                            go_right_a ? Path::PATH_LEFT : Path::PATH_RIGHT;
        ...
        if (fork_node) {
            ...
            for (auto it = fork_to_as_of.rbegin(); it != fork_to_as_of.rend();
                 it++)
                HASH_FUNCTION(it->hash, as_of_hash, as_of_hash);

            MERKLECPP_TRACE({
                MERKLECPP_TOUT << " - as_of hash: "
                               << as_of_hash.to_string(TRACE_HASH_SIZE)
                               << std::endl;
            });
        ...
        // The hashes along the path from the fork (now with new fork hash) to
        // the (past) root remains unchanged.
    ...
    void serialise(std::vector<uint8_t>& bytes) {
        ...
        serialise_uint64_t(leaf_nodes.size() + uninserted_leaf_nodes.size(),
                           bytes);
            ...
            for (size_t i = 0; i < level.size(); i += 2) {
                if (i + 1 >= level.size())
                    ...
                    next_level.push_back(
                        Node::make(level.at(i), level.at(i + 1)));
            }
    ...
    std::string to_string(size_t num_bytes = HASH_SIZE) const {
        ...
        if (!_root) {
            ...
            while (!level.empty()) {
                stream << level_no++ << ": ";
                for (auto n : level) {
                    stream << (n->dirty ? dirty_hash
                                        : n->hash.to_string(num_bytes));
  ...
  protected:
    ...
    const Node* leaf_node(size_t index) const {
        MERKLECPP_TRACE(MERKLECPP_TOUT << "> leaf_node " << index
                                       << std::endl;);
        if (index >= num_leaves())
            throw std::runtime_error("leaf index out of bounds");
        if (index - num_flushed >= leaf_nodes.size())
            return uninserted_leaf_nodes.at(index - num_flushed -
                                            leaf_nodes.size());
        else
    ...
    Node* process_insertion_stack(bool complete = true) {
        MERKLECPP_TRACE({
            ...
            for (size_t i = 0; i < insertion_stack.size(); i++)
                nodes += " " + insertion_stack.at(i).n->hash.to_string(
                                   TRACE_HASH_SIZE);
    ...
    void insert_leaves(bool complete = false) {
        if (!uninserted_leaf_nodes.empty()) {
            MERKLECPP_TRACE(MERKLECPP_TOUT << "* insert_leaves "
                                           << leaf_nodes.size() << " +"
                                           << uninserted_leaf_nodes.size()
                                           << std::endl;);
            // Potential future improvement: make this go fast when there are
            // many leaves to insert.
            for (auto& n : uninserted_leaf_nodes)
                insert_leaf(_root, n);
            uninserted_leaf_nodes.clear();
        }
